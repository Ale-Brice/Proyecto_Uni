...
async def register_detalle_producto(data: det_pro, db: AsyncSession = Depends(get_db)):
...
async def obtener_detalle_producto(db: AsyncSession = Depends(get_db)):
    det_pro = await obt_detalle_pro(db)
    return det_pro

@router.delete("/detalle_pro/{id}")
async def delete_detalle_producto(id: int, db: AsyncSession = Depends(get_db)):
...
async def update_detalle_producto(id: int, new_tallas: int, new_cantidad: int, new_fk_producto, db: AsyncSession = Depends(get_db)):
