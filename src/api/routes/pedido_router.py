...
async def register_pedido(data: pedido, db: AsyncSession = Depends(get_db)):
...
async def obtener_pedido(db: AsyncSession = Depends(get_db)):
    ped = await obt_pedido(db)
    return ped

@router.delete("/pedido/{id}")
async def delete_pedido(id: int, db: AsyncSession = Depends(get_db)):
...
async def update_pedido(id: int, new_fk_cliente: int, new_fk_producto: int, db: AsyncSession = Depends(get_db)):
