...
async def register_cost_adm(data: cost_adm, db: AsyncSession = Depends(get_audited_db)):
...
async def obtener_cost_adm(db: AsyncSession = Depends(get_audited_db)):
    cost_adm = await obt_cost_adm(db)
    return cost_adm

@router.delete("/cost_adm/{id}")
async def delete_cost_adm(id: int, db: AsyncSession = Depends(get_audited_db)):
...
async def update_cost_adm(id: int, new_tipo_costo : str, new_gasto_administrativo : int , new_fecha : date, db: AsyncSession = Depends(get_audited_db)):
