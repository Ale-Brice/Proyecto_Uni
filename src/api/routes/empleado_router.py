...
async def register_empleado(data: empleado, db: AsyncSession = Depends(get_audited_db)):
...
async def obtener_empleados(db: AsyncSession = Depends(get_audited_db)):
    emp = await obt_empleados(db)
    return emp

@router.delete("/empleados/{id}")
async def delete_empleado(id: int, db: AsyncSession = Depends(get_audited_db)):
...
async def update_empleado(id: int, new_name: str, apellido: str, numero: str, email: str, db: AsyncSession = Depends(get_audited_db)):
